        ...
        {config.map((item, index) => (
          <Card key={index}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              {item.icon=== Users?<Users className="h-4 w-4 text-muted-foreground" />:<h3 className="h-4 w-4 text-muted-foreground">Rs</h3>}
                ...
                {calculateTotalStudentsAndProfit().studentList.map(
                  (studentItem, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>Rs {studentItem.paidAmount}</TableCell>
                    </TableRow>
                  )
                )}
              </TableBody>
