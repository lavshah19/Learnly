...
const EsewaPaymentGateway = async (
  amount,
  ...
  let paymentData = {
    ...
    total_amount:
      Number(amount) + productDeliveryCharge + productServiceCharge + taxAmount,
    transaction_uuid,
  };
...
const EsewaCheckStatus = async (
  ...
  const response = await axios.get(ESEWAPAYMENT_STATUS_CHECK_URL, {
    params: paymentData,
  });
  if (!response) {
    return res.json({
